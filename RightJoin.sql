...
select *from
apartments a left join  residents r on a.apartment_id = r.apartment_id
where r.resident_id is null
;


-- EXERCISE 2: Count the number of residents per apartment
select a.apartment_id,count(r.apartment_id) from
apartments a left join  residents r on a.apartment_id = r.apartment_id
...
select *from
apartments a left join  residents r on a.apartment_id = r.apartment_id
left join maintenance_requests mr on r.apartment_id = mr.apartment_id;
...
select a.apartment_id, count(mr.apartment_id) from apartments a left join maintenance_requests mr on a.apartment_id = mr.apartment_id
group by(a.apartment_id)
;
